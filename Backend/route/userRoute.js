...
import { contactModel, userModel } from "../model/table.js";
...
router.put("/user-update", async (req, res) => {
  try {
    const { userId, name, email, password, contact, address } = req.body;
    const { profile } = req.files;
    profile.mv("uploads/" + profile?.name, (err) => {
      if (err) {
        res.json({
...
router.post("/login", async (req, res) => {
  try {
    ...
    console.log(isLogin, "aa raha hai");
...
router.post("/contact-list", async (req, res) => {
  try {
    const { email, name, contact, subject, message } = req.body;
    const isSend = await contactModel.findOne({ email });
    if (isSend) {
      res.json({
        code: 400,
